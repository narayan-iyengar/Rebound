...
from dataset_v3 import TripletDatasetV3, load_triplets, split_by_game, MODEL_H, MODEL_W
...
def parse_args():
    ...
    p.add_argument("--output", type=Path, default=Path(__file__).resolve().parent / "BallNetR_v3.mlpackage")
    return p.parse_args()
...
def main():
    ...
    # ── Data ─────────────────────────────────────────────────────────
    trip_path = TRAINING_ROOT / "labels" / "triplets_hires.jsonl"
    ...
    train_ds = TripletDatasetV3(train_recs, TRAINING_ROOT, augment=True)
    ...
    best_val = float("inf")
    best_ckpt = CKPT_DIR / "ballnetr_v3_best.pt"
    history: list[dict] = []
