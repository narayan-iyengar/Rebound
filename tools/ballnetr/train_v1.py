...
from dataset_v1 import TripletDataset, load_triplets, split_by_game
...
def parse_args():
    ...
    p.add_argument("--output", type=Path, default=Path(__file__).resolve().parent / "BallNetR_v1.mlpackage")
    return p.parse_args()
...
def focal_weighted_bce(pred: torch.Tensor, target: torch.Tensor, weight: torch.Tensor,
    ...
    """
    Weighted focal BCE — the target heatmap is mostly zeros, so we down-weight
    easy background pixels using the focal formulation.
    ...
    pos_loss = -alpha * (1 - p) ** gamma * target * torch.log(p)
    neg_loss = -(1 - alpha) * p ** gamma * (1 - target) * torch.log(1 - p)
    per_pixel = pos_loss + neg_loss                          # (B,1,H,W)
...
def main():
    ...
    train_ds = TripletDataset(train_recs, TRAINING_ROOT, augment=True)
    val_ds = TripletDataset(val_recs, TRAINING_ROOT, augment=False)
    ...
    best_val = float("inf")
    best_ckpt = CKPT_DIR / "ballnetr_v1_best.pt"
    history: list[dict] = []
