...
def parse_args():
    ...
    p.add_argument("--output", type=Path, default=Path(__file__).resolve().parent / "BallNetR_v0.mlpackage")
    return p.parse_args()
...
def main():
    ...
    train_ds = BallDataset(train_recs, TRAINING_ROOT, augment=True)
    ...
    best_val = float("inf")
    best_ckpt = CKPT_DIR / "ballnetr_v0_best.pt"
    history: list[dict] = []
